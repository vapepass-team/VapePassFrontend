...
/**
 * Normalize chat text before age checks (iOS/Safari curly apostrophes).
 */
...
/**
 * Client-side mirror of backend underage tripwire.
 * Used so the embed never enriches / continues shopping on underage admissions.
 */
export function detectsUnderage(message, legalAge = 19) {
  const text = normalizeAgeCheckText(message);
  if (!text) return false;

  const maxUnder = Math.max(1, Number(legalAge) - 1);
  const ages = [];
  for (let age = 1; age <= Math.min(maxUnder, 20); age += 1) ages.push(String(age));
  const ageAlt = ages.join('|');
  ...
  const patterns = [
    ...
    /\btoo\s+young\b/i,
    new RegExp(`\\bi\\s*(?:am|'m|m)\\s*(?:${ageAlt})\\b`, 'i'),
    new RegExp(`\\b(?:${ageAlt})\\s*(?:years?\\s*old|yo|yrs?)\\b`, 'i'),
    new RegExp(`\\b(?:my\\s*)?age\\s*(?:is\\s*)?(?:${ageAlt})\\b`, 'i'),
    new RegExp(`\\b(?:only|just)\\s*(?:${ageAlt})\\b`, 'i'),
    new RegExp(`\\bunder\\s*(?:the\\s*)?(?:age\\s*(?:of\\s*)?)?${legalAge}\\b`, 'i'),
    new RegExp(`\\bbelow\\s*(?:the\\s*)?(?:age\\s*(?:of\\s*)?)?${legalAge}\\b`, 'i'),
    new RegExp(`\\bi'?m\\s+not\\s+(?:${legalAge}|old\\s+enough)\\b`, 'i'),
    /\bunder\s*21\b/i,
    /\bi\s*(?:am|'m|m)\s*(?:ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen)\b/i,
  ];
...
export function detectsNewRecommendationPass(message, previousIntent = null) {
  ...
  const typeMatchers = [
    ...
    { key: 'prefilled', re: /\bpre-?filled\s*pods?\b/i },
    { key: 'cartridge', re: /\bcartridges?\b/i },
  ...
  if (found && previousIntent?.productType) {
    ...
    if (found.key === 'prefilled' && !prev.includes('prefilled') && !prev.includes('pre-filled')) return true;
    if (found.key === 'cartridge' && !prev.includes('cartridge')) return true;
